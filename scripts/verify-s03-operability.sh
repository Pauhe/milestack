...
# Default policy is fail-closed for production-like environments.
# rehearsal-local intentionally permits stale/degraded metadata truth while route guidance remains conservative.
if [[ -n "${REHEARSAL_ABORT_ALLOW_DEGRADED:-}" ]]; then
  ABORT_ALLOW_DEGRADED="${REHEARSAL_ABORT_ALLOW_DEGRADED}"
elif [[ "${DEPLOY_ENVIRONMENT}" == "rehearsal-local" ]]; then
  ABORT_ALLOW_DEGRADED="1"
else
  ABORT_ALLOW_DEGRADED="0"
fi

if [[ -n "${REHEARSAL_ABORT_REQUIRE_SYNC_STATUS:-}" ]]; then
  ABORT_REQUIRE_SYNC_STATUS="${REHEARSAL_ABORT_REQUIRE_SYNC_STATUS}"
elif [[ "${DEPLOY_ENVIRONMENT}" == "rehearsal-local" ]]; then
  ABORT_REQUIRE_SYNC_STATUS="stale"
else
  ABORT_REQUIRE_SYNC_STATUS="healthy"
fi

if [[ -n "${REHEARSAL_ABORT_MAX_STALE_HEALTH_SNAPSHOTS:-}" ]]; then
  ABORT_MAX_STALE_HEALTH_SNAPSHOTS="${REHEARSAL_ABORT_MAX_STALE_HEALTH_SNAPSHOTS}"
elif [[ "${DEPLOY_ENVIRONMENT}" == "rehearsal-local" ]]; then
  ABORT_MAX_STALE_HEALTH_SNAPSHOTS="3"
else
  ABORT_MAX_STALE_HEALTH_SNAPSHOTS="0"
fi
